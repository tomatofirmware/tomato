...
{
	return (jhash_2words(raddr, (u32) rport, rnd)
		& (TCP_SYNQ_HSIZE - 1));
}
					      ...
					      __u32 raddr, __u32 laddr)
{
	...
	for (prev = &lopt->syn_table[tcp_v4_synq_hash(raddr, rport, lopt->hash_rnd)]; /* SpeedMod linux-2.4.20-nethashfix */
	     (req = *prev) != NULL;
	     prev = &req->dl_next) {
...
static void tcp_v4_synq_add(struct sock *sk, struct open_request *req)
{
	struct tcp_opt *tp = &sk->tp_pinfo.af_tcp;
	struct tcp_listen_opt *lopt = tp->listen_opt;
	u32 h = tcp_v4_synq_hash(req->af.v4_req.rmt_addr, req->rmt_port, lopt->hash_rnd); /* SpeedMod linux-2.4.20-nethashfix */
