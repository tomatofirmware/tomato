...
/* !!TB - called on SIGUSR2 */
void pmap_user2(void)
{
	FILE *f;
	int n, r;
	unsigned short eport, iport, proto;
	char protocol[4], iaddr[32], desc[64];
	struct rule_state *tmp, *list;
	char ifname[IFNAMSIZ];
	u_int64_t packets, bytes;

	//
	if ((f = fopen("/var/spool/upnp.delete", "r")) != NULL) {
		r = fscanf(f, "%hu %hu", &proto, &eport);
		fclose(f);
		unlink("/var/spool/upnp.delete");
		if (r == 2) {
			if ((proto == 0) && (eport == 0)) {
				// delete all rules
				list = get_upnp_rules_state_list(0);
				n = 0;
				while(list)
				{
					/* remove the rule */
				...
				if (n > 0)
					syslog(LOG_NOTICE, "removed %d redirection rule(s)", n);
			}
			else {
				// delete single rule
	...
	do {
		...
		r = upnp_get_redirection_infos_by_index(n, &eport, protocol, &iport,
		                                        ...
		                                        desc, sizeof(desc) );
		if (r == 0) {
			fprintf(f, "%s\t[1,'%s',%u,%u,'%s','%s']", (n > 0) ? ",\n" : "",
				protocol, eport, iport, iaddr, desc);
			n++;
		}
	} while (r == 0);
	if (n > 0)
