...
void new_qoslimit_start(void)
{
	...
	int priority_num;
	char *lanipaddr; //!! RAF lan ip address
	char *lanmask; //!! RAF lan netmask
	char *waniface; //!! RAF wan interface 
	char *d_dlr; //!! RAF default DLrate 
	char *d_dlc; //!! RAF default DLceil 
	char *d_ulr; //!! RAF default ULrate 
	char *d_ulc; //!! RAF default ULceil 
	...
	lanipaddr = nvram_safe_get("lan_ipaddr"); //!!RAF
	lanmask = nvram_safe_get("lan_netmask"); //!!RAF
	waniface = nvram_safe_get("wan_iface"); //!! RAF
	d_dlr = nvram_safe_get("new_qoslimit_d_dlr"); //!! RAF 
	d_dlc = nvram_safe_get("new_qoslimit_d_dlc"); //!! RAF 
	d_ulr = nvram_safe_get("new_qoslimit_d_ulr"); //!! RAF 
	d_ulc = nvram_safe_get("new_qoslimit_d_ulc"); //!! RAF 
	...
	while (g) {
		...
		*/
		...
		if(atoi(tcplimit) > 0){
			if (strchr(ipaddr, '-') != NULL) {
			fprintf(f,
			"iptables -I FORWARD -m iprange --src-range %s -p tcp -m connlimit --connlimit-above %s -j DROP\n"
					"\n"
					,ipaddr,tcplimit);
	}
			else {
