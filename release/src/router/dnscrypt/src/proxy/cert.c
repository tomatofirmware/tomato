                   ...
                   const Bincert * const previous_bincert)
{
    ...
    serial = htonl(serial);
    logger(proxy_context, LOG_INFO, "Certificate #%lu received",
    ...
    logger_noformat(proxy_context, LOG_INFO, "Certificate valid");
...
cert_print_server_key(ProxyContext * const proxy_context)
{
    ...
    logger(proxy_context, LOG_INFO,
           "Server Key genuine fingerprint is %s", fingerprint);
}
