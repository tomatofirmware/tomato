...
<title>[<% ident(); %>] <% _("Bandwidth"); %>: <% _("Last 24 Hours"); %></title>
...
<div id='rstats'>
	...
	<div id='bwm-controls'>
		...
		Max:&nbsp;
			...
			<a href='javascript:switchScale(1)' id='scale1'><% _("Per IF"); %></a><br>
		Display:&nbsp;
...
if (nvram.rstats_enable != '1') {
	W('<div class="note-disabled"><% _("Bandwidth monitoring disabled"); %>.</b><br><br><a href="admin-bwm.asp"><% _("Enable"); %> &raquo;</a><div>');
	E('rstats').style.display = 'none';
}
else {
	W('<div class="note-warning" style="display:none" id="rbusy"><% _("The rstats program is not responding or is busy"); %>. <% _("Try reloading after a few seconds"); %>.</div>');
}
</script>
