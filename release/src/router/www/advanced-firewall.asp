...
<title>[<% ident(); %>] <% _("Advanced"); %>: <% _("Firewall"); %></title>
...
/* ICMP */
	...
	if ((enable_mcast) && (!E('_f_multicast_lan').checked) && (!E('_f_multicast_lan1').checked) && (!E('_f_multicast_lan2').checked) && (!E('_f_multicast_lan3').checked)) {
		ferror.set('_f_multicast', '<% _("IGMPproxy must be enabled in least one LAN bridge"); %>', quiet);
		return 0;
	} else {
...
<div class='section-title'><% _("Firewall"); %></div>
<div class='section'>
<script type='text/javascript'>
createFieldTable('', [
...
<div class='section-title'><% _("NAT"); %></div>
<div class='section'>
<script type='text/javascript'>
createFieldTable('', [
	{ title: '<% _("NAT loopback"); %>', name: 'nf_loopback', type: 'select', options: [[0,'All'],[1,'Forwarded Only'],[2,'Disabled']], value: fixInt(nvram.nf_loopback, 0, 2, 1) },
	{ title: '<% _("NAT target"); %>', name: 'ne_snat', type: 'select', options: [[0,'<% _("MASQUERADE"); %>'],[1,'<% _("SNAT"); %>']], value: nvram.ne_snat }
]);
</script>
</div>

<div class='section-title'><% _("Multicast"); %></div>
<div class='section'>
<script type='text/javascript'>
createFieldTable('', [
	{ title: '<% _("Enable IGMPproxy"); %>', name: 'f_multicast', type: 'checkbox', value: nvram.multicast_pass == '1' },
	{ title: '<% _("LAN"); %>', indent: 2, name: 'f_multicast_lan', type: 'checkbox', value: (nvram.multicast_lan == '1') },
	{ title: '<% _("LAN1"); %>', indent: 2, name: 'f_multicast_lan1', type: 'checkbox', value: (nvram.multicast_lan1 == '1') },
	{ title: '<% _("LAN2"); %>', indent: 2, name: 'f_multicast_lan2', type: 'checkbox', value: (nvram.multicast_lan2 == '1') },
	{ title: '<% _("LAN3"); %>', indent: 2, name: 'f_multicast_lan3', type: 'checkbox', value: (nvram.multicast_lan3 == '1') },
	null,
	{ title: '<% _("Enable Udpxy"); %>', name: 'f_udpxy_enable', type: 'checkbox', value: (nvram.udpxy_enable == '1') },
	{ title: '<% _("Enable client statistics"); %>', indent: 2, name: 'f_udpxy_stats', type: 'checkbox', value: (nvram.udpxy_stats == '1') },
	{ title: '<% _("Max clients"); %>', indent: 2, name: 'f_udpxy_clients', type: 'text', maxlen: 4, size: 6, value: fixInt(nvram.udpxy_clients || 3, 1, 5000, 3) },
	{ title: '<% _("Udpxy port"); %>', indent: 2, name: 'f_udpxy_port', type: 'text', maxlen: 5, size: 7, value: fixPort(nvram.udpxy_port, 4022) }
