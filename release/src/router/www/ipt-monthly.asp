...
<title>[<% ident(); %>] IP Traffic: Historia miesięczna</title>
...
dg.setup = function() {
	...
	this.headerSet(['Data', 'Host', 'Pobrano', 'Wysłano', 'Łącznie']);
}
	...
	if (monthly_history.length > 0) {
		...
		for (i = 0; i < monthly_history.length; ++i) {
			...
			if (E('_f_shortcuts').checked) {
				h = h + '<br><small>';
				h = h + '<a href="javascript:viewQosDetail(' + i + ')" title="Pokaż szczegłóy QoS">[qosdetails]</a>';
				h = h + '<a href="javascript:viewQosCTrates(' + i + ')" title="Pokaż limity transferu na każde IP">[qosrates]</a>';
...
function toggleVisibility(whichone) {
	if(E('sesdiv' + whichone).style.display=='') {
		...
		E('sesdiv' + whichone + 'showhide').innerHTML='(Kliknij aby pokazć)';
...
<div id='cstats'>
<div class='section-title'>IP Traffic: Historia miesięczna</div>
<div class='section'>
...
var c;
c = [];
c.push({ title: 'Pokaż tylko te adresy IP', name: 'f_filter_ip', size: 50, maxlen: 255, type: 'text', suffix: ' <small>(oddzielone spacją)</small>' });
...
c.push({ title: 'Format daty', name: 'f_dafm', type: 'select', options: [['0', 'yyyy-mm'], ['1', 'mm-yyyy'], ['2', 'mmm, yyyy'], ['3', 'mm.yyyy']] });
c.push({ title: 'Skala', name: 'f_scale', type: 'select', options: [['0', 'KB'], ['1', 'MB'], ['2', 'GB']] });
c.push({ title: 'Pokaż sumy w podsieciach', name: 'f_subnet', type: 'checkbox', suffix: ' <small>(Nie uwzględniane przy obliczaniu całkowitego ruchu w ostatniej linii)</small>' });
c.push({ title: 'Ukryj adresy IP bez ruchu', name: 'f_ignorezeroes', type: 'checkbox' });
