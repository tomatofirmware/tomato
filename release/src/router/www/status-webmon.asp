...
<title>[<% ident(); %>] <% _("Status"); %>: <% _("Web Usage"); %></title>
...
function clearLog(clear)
{
	...
	xob.onCompleted = function(text, xml) {
		xob = null;
		E('clear' + clear).innerHTML = '&raquo; <a href="javascript:clearLog(' + clear + ')"><% _("Clear"); %></a>';
	...
	E('clear' + clear).innerHTML = '<% _("Please wait"); %>... <img src="spin.gif" style="vertical-align:top">';
}
...
<div id='webmon' style='display:none'>
	<div id='webmon-domains'>
		<div class='section-title'><% _("Recently Visited Web Sites"); %></div>
		<div class='section'>
			<table id='dom-grid' class='tomato-grid' style="float:left" cellspacing=1></table>
			&raquo; <a href="webmon_recent_domains?_http_id=<% nv(http_id) %>"><% _("Download"); %></a>
			<div style="float:right;text-align:right;margin-right:5px" id="clear1">
				&raquo; <a href="javascript:clearLog(1)"><% _("Clear"); %></a>
			</div>
		</div>
	</div>

	<div id='webmon-searches'>
		<div class='section-title'><% _("Recent Web Searches"); %></div>
		<div class='section'>
			<table id='srh-grid' class='tomato-grid' style="float:left" cellspacing=1></table>
			&raquo; <a href="webmon_recent_searches?_http_id=<% nv(http_id) %>"><% _("Download"); %></a>
			<div style="float:right;text-align:right;margin-right:5px" id="clear2">
				&raquo; <a href="javascript:clearLog(2)"><% _("Clear"); %></a>
			</div>
		</div>
	</div>
	...
	<div id='webmon-controls'>
		<div id='webmon-mc'>
			...
			<a href='javascript:switchMaxCount(0);' id='mc0'><% _("All"); %></a>&nbsp;
			<small><% _("available entries"); %></small>
		</div>
		&raquo; <a href="admin-log.asp"><% _("Web Monitor Configuration"); %></a>
		<br><br>
...
<div id='wm-disabled'>
	<b><% _("Web Monitoring disabled"); %>.</b>
	<br><br>
	<a href="admin-log.asp"><% _("Enable"); %> &raquo;</a>
	<br><br>
</div>
