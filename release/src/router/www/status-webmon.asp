...
<title>[<% ident(); %>] Status: Monitor sieci</title>
...
function clearLog(clear)
{
	...
	xob.onCompleted = function(text, xml) {
		xob = null;
		E('clear' + clear).innerHTML = '&raquo; <a href="javascript:clearLog(' + clear + ')">Wyczy¶ć</a>';
	...
	E('clear' + clear).innerHTML = 'Proszę czekać... <img src="spin.gif" style="vertical-align:top">';
}
...
dg.setup = function() {
	...
	this.headerSet(['Czas ostatnich odwiedzin', 'Adres IP', 'Nazwa domeny']);
	this.sort(0);
}
...
sg.setup = function() {
	...
	this.headerSet(['Czas ostatniego szukania', 'Adres IP', 'Kryteria wyszukiwania']);
	this.sort(0);
}
...
<div id='webmon' style='display:none'>
	<div id='webmon-domains'>
		<div class='section-title'>Ostatnio odwiedzane strony WWW</div>
		<div class='section'>
			<table id='dom-grid' class='tomato-grid' style="float:left" cellspacing=1></table>
			&raquo; <a href="webmon_recent_domains?_http_id=<% nv(http_id) %>">Pobierz</a>
			<div style="float:right;text-align:right;margin-right:5px" id="clear1">
				&raquo; <a href="javascript:clearLog(1)">Wyczyść</a>
			</div>
		</div>
	</div>

	<div id='webmon-searches'>
		<div class='section-title'>Ostatnie wyniki wyszukiwania</div>
		<div class='section'>
			<table id='srh-grid' class='tomato-grid' style="float:left" cellspacing=1></table>
			&raquo; <a href="webmon_recent_searches?_http_id=<% nv(http_id) %>">Pobierz</a>
			<div style="float:right;text-align:right;margin-right:5px" id="clear2">
				&raquo; <a href="javascript:clearLog(2)">Wyczyść</a>
			</div>
		</div>
	</div>
	...
	<div id='webmon-controls'>
		<div id='webmon-mc'>
			...
			<a href='javascript:switchMaxCount(0);' id='mc0'>wszystkie</a>&nbsp;
			<small>dostępne wpisy</small>
		</div>
		&raquo; <a href="admin-log.asp">Konfiguracja Web Monitora</a>
		<br><br>
...
<div id='wm-disabled'>
	<b>Web Monitoring wyłączony.</b>
	<br><br>
	<a href="admin-log.asp">Włącz &raquo;</a>
	<br><br>
</div>
