...
<title>[<% ident(); %>] Status: Lista Urządzeń</title>
...
dg.populate = function()
{
	...
	for (i = dhcpd_lease.length - 1; i >= 0; --i) {
		a = dhcpd_lease[i];
		e = get(a[2], a[1]);
		e.lease = '<small><a href="javascript:deleteLease(\'L' + i + '\',\'' + a[1] + '\')" title="Zakończ dzierżawę" id="L' + i + '">' + a[3] + '</a></small>';
		e.name = a[0];
		e.ifname = '';
	}
	...
	for (i = list.length - 1; i >= 0; --i) {
		...
		if (e.mac.match(/^(..):(..):(..)/)) {
			b += '<br><small>' +
				'<a href="http://standards.ieee.org/cgi-bin/ouisearch?' + RegExp.$1 + '-' + RegExp.$2 + '-' + RegExp.$3 + '" target="_new" title="Szukaj OUI">[oui]</a> ' +
				'<a href="javascript:addStatic(' + i + ')" title="Dzierżawa statyczna...">[statycz]</a> ' +
				'<a href="javascript:addbwlimit(' + i + ')" title="BW Limiter">[limiter]</a>';

			if (e.rssi != '') {
				b += ' <a href="javascript:addWF(' + i + ')" title="Filtrowanie WiFi...">[wfilter]</a>';
			}
			b += '</small>';
		}
		else {
...
dg.setup = function()
{
	...
	this.headerSet(['Interfejs', 'Adres MAC', 'Adres IP', 'Nazwa', 'RSSI', 'Jakość', 'Transfer TX/RX', 'Dzierżawa']);
	this.populate();
	this.sort(2);
}
...
<div class='section-title'>Lista urządzeń</div>
<div class='section'>
	...
	if (nvram['wl'+u+'_radio'] == '1') {
		if (wl_sunit(uidx)<0) {
			...
			if ((nvram['wl'+u+'_mode'] == 'ap') || (nvram['wl'+u+'_mode'] == 'wds'))
				a = '&nbsp;&nbsp;&nbsp; <input type="button" value="Pomiar" onclick="javascript:window.location=\'wlmnoise.cgi?_http_id=' + nvram.http_id + '&_wl_unit=' + u +'\'">';
			f.push( { title: '<b>Poziom szumów tła (' + wl_ifaces[uidx][0] + ')&nbsp;:</b>',
