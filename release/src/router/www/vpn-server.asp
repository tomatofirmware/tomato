...
<title>[<% ident(); %>] OpenVPN: Serwer</title>
...
tabs = [['server1', 'Serwer 1'],['server2', 'Serwer 2']];
sections = [['basic', 'Podstawowe'],['advanced', 'Zaawansowane'],['keys','Klucze'],['status','Status']];
ccdTables = [];
statusUpdaters = [];
...
ciphers = [['default','Domyślny'],['none','Brak']];
...
function updateStatus(num)
{
	...
	{
		statusUpdaters[num].errors.innerHTML += 'BŁĄD! '+ex+'<br>';
		xob = null;
	}
...
{
	if (changed && !confirm("Niezapisane zmiany zostaną utracone. Czy na pewno chcesz kontynuować?")) return;
...
CCDGrid.prototype.verifyFields = function(row, quiet)
{
	...
	if (f[1].value == "") { ferror.set(f[1], "Podanie nazwy jest obowiązkowe.", quiet); ret = 0; }
	if (f[1].value.indexOf('>') >= 0 || f[1].value.indexOf('<') >= 0) { ferror.set(f[1], "Nazwa nie może zawierać znaków '<' or '>' ", quiet); ret = 0; }
	if (f[2].value != "" && !v_ip(f[2],quiet,0)) ret = 0;
	if (f[3].value != "" && !v_netmask(f[3],quiet)) ret = 0;
	if (f[2].value == "" && f[3].value != "" ) { ferror.set(f[2], "Należy wprowadzić zarówno podsieć i maskę sieciową lub pozostawić puste oba pola.", quiet); ret = 0; }
	if (f[3].value == "" && f[2].value != "" ) { ferror.set(f[3], "Należy wprowadzić zarówno podsieć i maskę sieciową lub pozostawić puste oba pola.", quiet); ret = 0; }
	if (f[4].checked && (f[2].value == "" || f[3].value == "")) { ferror.set(f[4], "Nie można wymusić routingu jeśli nie został podany. Wprowadź podsieć/maskę sieciową", quiet); ret = 0; }
...
function init()
{
	...
	for (i = 0; i < tabs.length; ++i)
	{
		...
		ccdTables[i].headerSet(['Włączone', 'Nazwa', 'Podsieć', 'Maska', 'Push']);
...
<div class='section-title'>Konfiguracja serwera OpenVPN</div>
<div class='section'>
<script type='text/javascript'>
...
for (i = 0; i < tabs.length; ++i)
{
	...
	W('<div id=\''+t+'-basic\'>');
	createFieldTable('', [
		{ title: 'Startuj razem z WAN', name: 'f_vpn_'+t+'_eas', type: 'checkbox', value: nvram.vpn_server_eas.indexOf(''+(i+1)) >= 0 },
		{ title: 'Typ interfejsu', name: 'vpn_'+t+'_if', type: 'select', options: [ ['tap','TAP'], ['tun','TUN'] ], value: eval( 'nvram.vpn_'+t+'_if' ) },
		{ title: 'Protokół', name: 'vpn_'+t+'_proto', type: 'select', options: [ ['udp','UDP'], ['tcp-server','TCP'] ], value: eval( 'nvram.vpn_'+t+'_proto' ) },
		{ title: 'Port', name: 'vpn_'+t+'_port', type: 'text', value: eval( 'nvram.vpn_'+t+'_port' ) },
		{ title: 'Firewall', name: 'vpn_'+t+'_firewall', type: 'select', options: [ ['auto', 'Automatycznie'], ['external', 'Tylko zewnętrzny'], ['custom', 'Własny'] ], value: eval( 'nvram.vpn_'+t+'_firewall' ) },
		{ title: 'Tryb uwierzytelniania', name: 'vpn_'+t+'_crypt', type: 'select', options: [ ['tls', 'TLS'], ['secret', 'Klucz statyczny'], ['custom', 'Własny'] ], value: eval( 'nvram.vpn_'+t+'_crypt' ),
			suffix: '<span id=\''+t+'_custom_crypto_text\'>&nbsp;<small>(należy skonfigurować ręcznie...)</small></span>' },
		{ title: 'Dodatkowe uwierzytelnianie HMAC (tls-auth)', name: 'vpn_'+t+'_hmac', type: 'select', options: [ [-1, 'Wyłączone'], [2, 'Dwukierunkowe'], [0, 'Przychodzące (0)'], [1, 'Wychodzące (1)'] ], value: eval( 'nvram.vpn_'+t+'_hmac' ) },
		{ title: 'Podsieć/maska VPN', multi: [
		...
		{ title: 'Pula adresów klientów', multi: [
		...
		{ title: 'Lokalne/zdalne adresy stacji końcowych', multi: [
	...
	W('<div id=\''+t+'-advanced\'>');
	createFieldTable('', [
		{ title: 'Interwał próbkowania', name: 'vpn_'+t+'_poll', type: 'text', maxlen: 4, size: 5, value: eval( 'nvram.vpn_'+t+'_poll' ), suffix: '&nbsp;<small>(w minutach, 0 - wyłączone)</small>' }, 
		{ title: 'Dodaj u klientów routing do LANu (Push to LAN)', name: 'f_vpn_'+t+'_plan', type: 'checkbox', value: eval( 'nvram.vpn_'+t+'_plan' ) != 0 },
		{ title: 'Przekieruj ruch klientów do Internetu przez VPN', name: 'f_vpn_'+t+'_rgw', type: 'checkbox', value: eval( 'nvram.vpn_'+t+'_rgw' ) != 0 },
		{ title: 'Odpowiedz jako DNS', name: 'f_vpn_'+t+'_dns', type: 'checkbox', value: nvram.vpn_server_dns.indexOf(''+(i+1)) >= 0 },
		{ title: 'Przekaż DNS do klientów', name: 'f_vpn_'+t+'_pdns', type: 'checkbox', value: eval( 'nvram.vpn_'+t+'_pdns' ) != 0 },
		{ title: 'Typ klucza szyfrującego', name: 'vpn_'+t+'_cipher', type: 'select', options: ciphers, value: eval( 'nvram.vpn_'+t+'_cipher' ) },
		{ title: 'Kompresja', name: 'vpn_'+t+'_comp', type: 'select', options: [ ['-1', 'Wyłączona'], ['no', 'Brak'], ['yes', 'Włączona'], ['adaptive', 'Adaptacyjna'] ], value: eval( 'nvram.vpn_'+t+'_comp' ) },
		{ title: 'Czas renegocjacji TLS', name: 'vpn_'+t+'_reneg', type: 'text', maxlen: 10, size: 7, value: eval( 'nvram.vpn_'+t+'_reneg' ),
			suffix: '&nbsp;<small>(w sekundach, domyślnie -1)</small>' },
		{ title: 'Opcje dla wybranych klientów', name: 'f_vpn_'+t+'_ccd', type: 'checkbox', value: eval( 'nvram.vpn_'+t+'_ccd' ) != 0 },
		{ title: 'Zezwól na ruch Klient<->Klient', name: 'f_vpn_'+t+'_c2c', type: 'checkbox', value: eval( 'nvram.vpn_'+t+'_c2c' ) != 0 },
		{ title: 'Zezwól tylko tym klientom', name: 'f_vpn_'+t+'_ccd_excl', type: 'checkbox', value: eval( 'nvram.vpn_'+t+'_ccd_excl' ) != 0 },
		{ title: '', suffix: '<table class=\'tomato-grid\' id=\'table_'+t+'_ccd\'></table>' },
		{ title: 'Własna konfiguracja', name: 'vpn_'+t+'_custom', type: 'textarea', value: eval( 'nvram.vpn_'+t+'_custom' ) }
	]);
	W('</div>');
	W('<div id=\''+t+'-keys\'>');
	W('<p class=\'keyhelp\'>Aby dowiedzieć się jak wygenerować klucze należy odwiedzić stronę OpenVPN <a id=\''+t+'-keyhelp\'>HOWTO</a>.</p>');
	createFieldTable('', [
		{ title: 'Klucz statyczny', name: 'vpn_'+t+'_static', type: 'textarea', value: eval( 'nvram.vpn_'+t+'_static' ) },
		{ title: 'Certyfikat CA (Certificate Authority)', name: 'vpn_'+t+'_ca', type: 'textarea', value: eval( 'nvram.vpn_'+t+'_ca' ) },
		{ title: 'Certyfikat serwera', name: 'vpn_'+t+'_crt', type: 'textarea', value: eval( 'nvram.vpn_'+t+'_crt' ) },
		{ title: 'Klucz prywatny serwera', name: 'vpn_'+t+'_key', type: 'textarea', value: eval( 'nvram.vpn_'+t+'_key' ) },
		{ title: 'Parametry Diffie Hellman', name: 'vpn_'+t+'_dh', type: 'textarea', value: eval( 'nvram.vpn_'+t+'_dh' ) }
	]);
	W('</div>');
	W('<div id=\''+t+'-status\'>');
		W('<div id=\''+t+'-no-status\'><p>Serwer nie działa lub status nie może zostać odczytany.</p></div>');
		W('<div id=\''+t+'-status-content\' style=\'display:none\' class=\'status-content\'>');
			W('<div id=\''+t+'-status-header\' class=\'status-header\'><p>Dane aktualne na dzień <span id=\''+t+'-status-time\'></span>.</p></div>');
			W('<div id=\''+t+'-status-clients\'><div class=\'section-title\'>Lista klientów</div><table class=\'tomato-grid status-table\' id=\''+t+'-status-clients-table\'></table><br></div>');
			W('<div id=\''+t+'-status-routing\'><div class=\'section-title\'>Tablica routingu</div><table class=\'tomato-grid status-table\' id=\''+t+'-status-routing-table\'></table><br></div>');
			W('<div id=\''+t+'-status-stats\'><div class=\'section-title\'>Statystyka ogólna</div><table class=\'tomato-grid status-table\' id=\''+t+'-status-stats-table\'></table><br></div>');
			W('<div id=\''+t+'-status-errors\' class=\'error\'></div>');
		W('</div>');
		W('<div style=\'text-align:right\'><a href=\'javascript:updateStatus('+i+')\'>Odśwież status</a></div>');
	W('</div>');
	W('<input type="button" value="' + (eval('vpn'+(i+1)+'up') ? 'Zatrzymaj' : 'Uruchom') + ' teraz" onclick="toggle(\'vpn'+t+'\', vpn'+(i+1)+'up)" id="_vpn'+t+'_button">');
	W('</div>');
}
