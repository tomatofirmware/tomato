...
for (i = 0; i < tabs.length; ++i)
{
	...
	W('</div>');
	W('<div id=\''+t+'-keys\'>');
	W('<p class=\'keyhelp\'>For help generating keys, refer to the OpenVPN <a id=\''+t+'-keyhelp\'>HOWTO</a>. To reuse a certificate from another server instance, just enter its number.</p>');
	createFieldTable('', [
