...
<title>[<% ident(); %>] QoS: Zestawienie transferów dla poszczególnych połączeń</title>
		...
		for (j = cols.length-1; j >= 0; j--) {
			if (data[cols[j]].indexOf(ip) != -1 ) {
				...
				if (E('_f_shortcuts').checked)
					data[cols[j]] = data[cols[j]] + ' <small><a href="javascript:addExcludeList(\'' + ip + '\')" title="Usuń z listy">[Ukryj]</a></small>';
...
grid.setup = function() {
	this.init('grid', 'sort');
	this.headerSet(['Protokół', 'Adres<br>źródłowy', ' Port źródłowy', 'Adres<br>docelowy', 'Port docelowy', 'Wysyłanie', 'Pobieranie']);
}
...
ref.refresh = function(text)
{
	...
	for (i = 0; i < ctrate.length; ++i) {
		...
		for (j = cols.length-1; j >= 0; j--) {
			...
			if (E('_f_shortcuts').checked) {
				if (cache[ip] == null) {
					b[cols[j]] = b[cols[j]] + ' <small><a href="javascript:addToResolveQueue(\'' + ip + '\')" title="Pokaż nazwę hosta dla tego adresu IP">[pokaż nazwę]</a></small>';
				}
				b[cols[j]] = b[cols[j]] + ' <small><a href="javascript:addExcludeList(\'' + ip + '\')" title="Odfiltruj to IP">[ukryj]</a></small>';
			}
		}
	...
	if (numconnshown != numconntotal)
		E('numtotalconn').innerHTML='<small><i>(showing ' + numconnshown + ' z ' + numconntotal + ' połączeń)</i></small>';
	else
		E('numtotalconn').innerHTML='<small><i>(' + numconntotal + ' połączeń)</i></small>';
}
...
<div class='section-title' id='stitle' onclick='document.location="qos-graphs.asp"' style='cursor:pointer'>Prędkość transferu: <span id='numtotalconn'></span></div>
<div class='section'>
<table id='grid' class='tomato-grid' style="float:left" cellspacing=1></table>

<div id='loading'><br><b>Ładuję...</b></div>
</div>

<!-- / / / -->

<div class='section-title'>Filtry: <small><i><a href='javascript:toggleVisibility("filters");'><span id='sesdivfiltersshowhide'>(Pokaż/Ukryj)</span></a></i></small></div>
<div class='section' id='sesdivfilters' style='display:none'>
<script type='text/javascript'>
var c;
c = [];
c.push({ title: 'Tylko wybrane adresy IP', name: 'f_filter_ip', size: 50, maxlen: 255, type: 'text', suffix: ' <small>(lista oddzielona przecinkami)</small>' });
c.push({ title: 'Wyklucz te adresy IP', name: 'f_filter_ipe', size: 50, maxlen: 255, type: 'text', suffix: ' <small>(lista oddzielona przecinkami)</small>' });
c.push({ title: 'Wyklucz ruch do bramy', name: 'f_excludegw', type: 'checkbox', value: ((nvram.t_hidelr) == '1' ? 1 : 0) });
c.push({ title: 'Wyklucz IPv4 broadcast', name: 'f_excludebcast', type: 'checkbox' });
c.push({ title: 'Wyklucz IPv4 multicast', name: 'f_excludemcast', type: 'checkbox' });
c.push({ title: 'Ignoruj połączenia nieaktywne', name: 'f_excludebythreshold', type: 'checkbox' });
c.push({ title: 'Automatycznie zamieniaj IP na nazwy', name: 'f_autoresolve', type: 'checkbox' });
